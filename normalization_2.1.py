...
first = True # флаг для того, чтобы узнать максимумы у замедленной флуоресценнции от которой отняли искажающую кривую времени жизни
...
for x in range(len(dl_fl_y[0])):
    ...
    for y in range(NUM_OF_EXP):
        dl_fl_y[y][x] = dl_fl_y[y][x] - dl_fl_y_wo_salt[y][-x - 1]
        if first:
...
out_obj.param_out(result_bi_exp,
                  common_name,
                  NUM_OF_EXP,
                  head = "The best fitted parameters for biexponential model for delayed fluorescence")
out_obj.param_out(result_dl_fl_sing_exp,
                  common_name,
                  NUM_OF_EXP,
                  head = "The best fitted parameters for monoexponential model for delayed fluorescence")
common_name.reverse()
out_obj.param_out(result_ph_sing_exp,
                  common_name,
                  NUM_OF_EXP,
                  head = "The best fitted parameters for exponential model for phosphorescence")
